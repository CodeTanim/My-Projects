 ...
 * address (the pointer that tells this function where it came from / what
 * it should do when finished), and the program will crash, likely with a
 * seg fault.  There are some diagrams here which might be helpful:
 * https://eli.thegreenplace.net/2011/09/06/stack-frame-layout-on-x86-64
 * */
/* SPOOKY STUFF:
 * The ability to overwrite the return address has been (ab)used to great
 ...
 * the return address (there are some tricks to make this easier than it would
 * seem at first), you can convince the pwdcheck function to execute it!  You
 * can also try to set the return address to a library function, maybe to start
 * a shell (this is called the "return to libc" attack).
 * NOTE: there are a number of defenses (see below about "canaries") which are
 * thankfully enabled by default in modern gcc.
 * */
 ...
 * smashing detected" when the program crashes.
 * If you are curious about how this is implemented, you can browse the
 * assembly output I left (the .s files).  (There are also makefile targets
 * if you want to make them yourself.)  File call-stack-c.s is assembly where
 * stack canaries are enabled; call-stack.s is without.
 * */
